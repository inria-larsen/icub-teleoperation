...
#include "comRetargetingModule.h"
...
comRetargetingModule::comRetargetingModule()
	:  m_robot(0)
{
    comThread      = 0;
    period          = 10;
...
bool comRetargetingModule::attach(yarp::os::Port &source) 
{
    return this->yarp().attachAsServer(source);
}


bool comRetargetingModule::configure(yarp::os::ResourceFinder &rf) 
{
    if( rf.check("robot") ) {
	robotName = rf.find("robot").asString();
    } else {
	std::cerr << "comRetargetingModule::configure failed: robot parameter not found. Closing module." << std::endl;
	return false;
    }
	...
	std::cerr << "comRetargetingModule::configure failed: name parameter not found. Closing module." << std::endl;
	return false;
    }
    ...
    {
	fprintf(stderr,"[ERR] comRetargetingThread: impossible to open wholeBodyInterface: wbi_conf_file option missing");
	return false;
    }
	...
	{
	    fprintf(stderr, "[ERR] comRetargetingModule: impossible to load wbiId joint list with name %s\n",RobotDynamicModelJointsListName.c_str());
	    return false;
	}
    }
    ...
    //--------------------------COM RETARGETING THREAD--------------------------
    comThread = new comRetargetingThread(moduleName,
	                                    robotName,
	                                    period,
					    nDOFs,
					    *m_robot,
    ...
    yInfo("comRetargetingThread started. (Running at %d ms)",period);
...
bool comRetargetingModule::interruptModule()      
{
    if(comThread)
	comThread->suspend();
    rpcPort.interrupt();
...
bool comRetargetingModule::close()                                 
{
    ...
    if(comThread)
    {
	yInfo() << getName() << ": closing comRetargetingThread";
	comThread->stop();
	delete comThread;
	comThread = 0;
    }
	...
	printf("Next time you could set a lower period to improve the comRetargeting performance.\n");
...
bool comRetargetingModule::updateModule()
{
    if (comThread==0)
    {
	yError("comRetargetingThread pointers are zero\n");
	return false;
    }
    ...
    {
	yWarning("[WARNING] comRetargeting loop is too slow. Real period: %3.3f+/-%3.3f. Expected period %d.\n", avgTime, stdDev, period);
...
bool comRetargetingModule::quit()
{
    return this->close();
}
