...
jointMappingModule::jointMappingModule()
	:  m_robot(0)
{
    jointThread      = 0;
    offset 	    = 0.1;
...
bool jointMappingModule::configure(yarp::os::ResourceFinder &rf) 
{
    if( rf.check("robot") ) {
...
bool jointMappingModule::interruptModule()                       
{
    if(jointThread)
	jointThread->suspend();
    rpcPort.interrupt();
...
bool jointMappingModule::updateModule()
{
    if (jointThread==0)
    {
