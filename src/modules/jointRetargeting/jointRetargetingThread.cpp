...
//===============================================
//        JointRetargeting Main THREAD
//===============================================

jointRetargetingThread::jointRetargetingThread(string _name,
			       string _robotName,
			       int _actuatedDOFs,
       ...
       m_maxJointLimits(_actuatedDOFs)
{
    ...
    yInfo() << "Launching jointRetargetingThread with name : " << _name << " and robotName " << _robotName << " and period " << _period;
   
}


bool jointRetargetingThread::threadInit()
{  
    initJoint();
    ...
    yInfo() << "jointRetargetingThread::threadInit finished successfully.";

    return true;
}


bool jointRetargetingThread::initJoint()
{
    //limits
    bool result = false;
...
void jointRetargetingThread::publishJoints()
{
...
//----------------------------
void jointRetargetingThread::getRobotJoints()
{
...
void jointRetargetingThread::avoidJointLimits() 
{
    for (int i = 0; i < jointPos.size(); i++){ 
...
//------ RUN ------
void jointRetargetingThread::run()
{
    //if( this->run_mutex_acquired )
    //{
	//yError() << "jointRetargeting: run_mutex already acquired at the beginning of run method.";
...
void jointRetargetingThread::mapping_run()
{
...
void jointRetargetingThread::threadRelease()
{
    //run_mutex.lock();
