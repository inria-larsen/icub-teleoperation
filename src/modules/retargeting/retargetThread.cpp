...
void retargetThread::publishJoints()
{
    ...
    for (int i=0; i < qActuated.size(); i++){
        output.addDouble(qActuated(i));
    }
...
void retargetThread::getRobotJoints()
{
    ...
    if ((joint_list.compare("ROBOT_TORQUE_CONTROL_JOINTS_WITHOUT_PRONOSUP") == 0) && (controller.compare("TorqueBalancing") == 0)){
       ...
       for (int i=7; i<11; i++)
	    qActuated(i)=q(i+6);
       for (int i=11; i<qActuated.size(); i++)
	    qActuated(i)=q(i+9);
    }
    ...
    // Joint limits for all the joints
    if (this->m_checkJointLimits){
	// Avoid joint limits
	for (int i = 0; i < qActuated.size(); i++){ 
	if (qActuated(i) <= (m_minJointLimits(i) + offset))		//check min
		qActuated(i) = m_minJointLimits(i) + offset;
	if (qActuated(i) >= (m_maxJointLimits(i) - offset))		//check max
		qActuated(i) = m_maxJointLimits(i) - offset;
	}    
	if ((joint_list.compare("ROBOT_TORQUE_CONTROL_JOINTS_WITHOUT_PRONOSUP") == 0) && (controller.compare("TorqueBalancing") == 0)){
	       for (int i=0; i<3; i++)
		    q(i)=qActuated(i);
